...
export default async function EditImagePage({ searchParams }: { searchParams: Promise<{ source?: string; fromJob?: string }> }) {
  ...
  const models = discovered.filter((model) => model.workflowType === "image-edit" && model.visible).map((model) => ({ key: model.key, logicalKey: model.logicalKey, displayName: model.displayName, availability: model.availability, capabilities: model.capabilities, maxReferenceImages: model.maxReferenceImages, maxReferenceImagesWithSource: referenceImageLimit(model, true), controls: getGenerationControls(model.schema, model.defaults, { workflow: "image", fallbackResolutions: getImageFallbackResolutions(model.logicalKey), fallbackResolution: typeof model.defaults.resolution === "string" ? model.defaults.resolution : "1024x1024" }), loraCatalog: model.loraCatalog, defaultLoras: preferences.defaultLoras[`image-edit:${model.key}`] ?? preferences.defaultLoras[`image-edit:${model.logicalKey}`] ?? [] }));
