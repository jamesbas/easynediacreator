...
export default async function EditImagePage({ searchParams }: { searchParams: Promise<{ source?: string; fromJob?: string }> }) {
  ...
  const models = discovered.filter((model) => model.workflowType === "image-edit").map((model) => ({ key: model.key, displayName: model.displayName, availability: model.availability, controls: getGenerationControls(model.schema, model.defaults, { workflow: "image", fallbackResolutions: [], fallbackResolution: typeof model.defaults.resolution === "string" ? model.defaults.resolution : "1024x1024" }), loraCatalog: model.loraCatalog }));
