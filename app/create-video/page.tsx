...
export default async function CreateVideoPage({ searchParams }: { searchParams: Promise<{ start?: string; fromJob?: string }> }) {
  ...
  const models = discovered.filter((model) => model.workflowType === "video-create").map((model) => ({ key: model.key, displayName: model.displayName, availability: model.availability, controls: getGenerationControls(model.schema, model.defaults, { workflow: "video", fallbackResolutions: [], fallbackResolution: typeof model.defaults.resolution === "string" ? model.defaults.resolution : "1280x720" }), defaultSourceStrength: typeof model.defaults.input_video_strength === "number" ? model.defaults.input_video_strength : 0.85, supportsEndFrame: model.capabilities.includes("end-frame"), loraCatalog: model.loraCatalog }));
