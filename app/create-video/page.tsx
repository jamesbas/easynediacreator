...
export default async function CreateVideoPage({ searchParams }: { searchParams: Promise<{ start?: string; fromJob?: string }> }) {
  ...
  // The legacy `ltx-2` mapping still owns whatever default LoRAs were saved
  // against it; it belongs to the LTX checkpoint that mapping resolves to, not
  // to whichever model happens to sort first now that H3 is the default.
