...
export function buildLtx2VideoSettings(request: VideoCreateRequest, defaults: Record<string, unknown>, schema: Record<string, unknown>, modelType: string, startPath: string, endPath?: string) {
  ...
  setDiscoveredSetting(settings, schema, defaults, modelType, ["num_inference_steps", "steps"], request.steps);
  ...
  setDiscoveredSetting(settings, schema, defaults, modelType, ["input_video_strength"], request.sourceStrength, true);
