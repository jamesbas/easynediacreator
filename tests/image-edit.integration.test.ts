...
describe("image editing", () => {
  ...
  it("edits a validated uploaded image through the GPU queue", async () => {
    ...
    const created = await editImage({ sourceUploadId: upload.id, prompt: "Turn the sky teal", negativePrompt: "blurry, malformed hands", modelKey: "qwen-image-edit", loras: [], advanced: {} });
    ...
    expect(client.getLastSubmissionForTests()?.settings).toMatchObject({ negative_prompt: "blurry, malformed hands" });
  }, 5000);
});