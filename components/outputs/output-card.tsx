...
export function OutputCard({ asset, priority = false }: { asset: ReturnType<typeof publicAsset>; priority?: boolean }) {
  return (
    <article className="overflow-hidden border border-[var(--line)] bg-[var(--surface)]">
      ...
      <div className="p-4"><div className="flex items-center justify-between gap-3"><span className="font-mono text-[0.66rem] font-semibold uppercase text-[var(--teal)]">{asset.type}</span><time className="text-xs text-[var(--muted)]">{new Date(asset.createdAt).toLocaleDateString()}</time></div><p className="mt-3 line-clamp-2 text-sm leading-6">{asset.prompt}</p><GenerationSummaryChips summary={asset.summary} modelKey={asset.modelKey} className="mt-3" /><AssetActions asset={asset} /></div>
    </article>
  );
}