...
type FormModel = { key: string; displayName: string; availability: string; reason?: string; controls: GenerationControls; lockedGuidance?: number; maxReferenceImages?: number; loraCatalog: LoraCatalog };
...
export function ImageCreateForm({ models, assets, characterReferences, defaultModel, characterPrompt, initialRequest }: { models: FormModel[]; assets: ReferenceAssetOption[]; characterReferences: CharacterReferenceSummary[]; defaultModel: string; characterPrompt: string; initialRequest?: ImageCreateRequest }) {
  ...
  const referenceLimit = selected?.maxReferenceImages ?? Math.max(0, ...referenceCapableModels.map((model) => model.maxReferenceImages ?? 0));
  const referencesUnavailable = !referenceCapableModels.length ? "No installed model accepts reference images." : !selected?.maxReferenceImages ? `${selected?.displayName ?? "This model"} renders from text alone. Choose ${referenceCapableModels.map((model) => model.displayName).join(" or ")} to use reference images.` : undefined;
  ...
  return (
    <form className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_300px]" onSubmit={async (event) => {
      ...
      try {
        ...
        setError(submissionError instanceof Error ? submissionError.message : "Generation could not be started.");
        setSubmitting(false);
      }
    }}>
      ...
      <aside className="space-y-5 border border-[var(--line)] bg-[var(--surface)] p-5">
        ...
        {referenceCount > 0 && !selected?.maxReferenceImages && <p className="-mt-3 text-[0.68rem] leading-4 text-[var(--accent)]">This model renders from text alone. Remove the references or choose a reference-capable model.</p>}
        ...
        {guidanceLocked && <p className="-mt-3 text-[0.68rem] leading-4 text-[var(--muted)]">Fixed at {lockedGuidance} for this model&apos;s distilled inference.</p>}
