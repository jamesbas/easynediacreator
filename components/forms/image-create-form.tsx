...
export function ImageCreateForm({ models, defaultModel, characterPrompt, initialRequest }: { models: FormModel[]; defaultModel: string; characterPrompt: string; initialRequest?: ImageCreateRequest }) {
  ...
  const [guidanceScale, setGuidanceScale] = useState(reusableModel ? initialRequest?.guidanceScale ?? reusableModel.defaultGuidance : selected?.defaultGuidance ?? 1);
  const [steps, setSteps] = useState(reusableModel ? initialRequest?.steps ?? reusableModel.defaultSteps : selected?.defaultSteps ?? 20);
  ...
  const previousRecipeValues = useRef({ steps: reusableModel ? initialRequest?.steps ?? reusableModel.defaultSteps : selected?.defaultSteps ?? 20, guidance: reusableModel ? initialRequest?.guidanceScale ?? reusableModel.defaultGuidance : selected?.defaultGuidance ?? 1 });
  ...
  return (
    <form className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_300px]" onSubmit={async (event) => {
      ...
      <aside className="space-y-5 border border-[var(--line)] bg-[var(--surface)] p-5">
        <Field label="Model"><select value={modelKey} onChange={(event) => { const next = models.find((model) => model.key === event.target.value); setModelKey(event.target.value); setGuidanceScale(next?.defaultGuidance ?? 1); setSteps(next?.defaultSteps ?? 20); setPreset(undefined); setSelectedLoraNames([]); setReuseSelections(false); }} required className="control"><option value="" disabled>No model available</option>{models.map((model) => <option key={model.key} value={model.key} disabled={model.availability !== "available"}>{model.displayName}{model.availability !== "available" ? ` (${model.availability})` : ""}</option>)}</select></Field>
        {selected?.reason && <p className="text-xs leading-5 text-[var(--accent)]">{selected.reason}</p>}
        <Field label="Resolution"><select name="resolution" defaultValue={reuseSelections ? initialRequest?.resolution ?? selected?.defaultResolution : selected?.defaultResolution} key={modelKey} className="control">{(selected?.resolutions.length ? selected.resolutions : [selected?.defaultResolution ?? "1024x1024"]).map((resolution) => <option key={resolution}>{resolution}</option>)}</select></Field>
        ...
        <details className="border-t border-[var(--line)] pt-4"><summary className="cursor-pointer text-sm font-bold">Advanced</summary><div className="mt-4"><Field label="Seed"><input className="control" name="seed" type="number" min="0" max="2147483647" placeholder="Random" defaultValue={initialRequest?.seed} /></Field></div></details>
